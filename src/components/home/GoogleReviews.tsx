...
const BASE_SCROLL_SPEED = 0.045;

const ReviewStars = ({ size = 16 }: { size?: number }) => (
  <div className="flex gap-0.5 mb-4">
    {[...Array(5)].map((_, index) => (
      <Star key={index} size={size} className="fill-gold text-gold" />
    ))}
  </div>
);
...
const ReviewCard = ({
  ...
  return (
    <article className="flex h-full w-[22rem] shrink-0 flex-col rounded-sm border border-border bg-card p-6 shadow-luxury transition-shadow duration-300 hover:shadow-luxury-lg md:w-[24rem] lg:w-[25rem]">
      <ReviewStars />
      <div className="mb-4 flex-1">
        ...
        {isLong && (
          <button
            ...
            className="mt-3 inline-flex items-center gap-2 text-xs font-sans uppercase tracking-[0.18em] text-gold transition-colors hover:text-gold-light"
          >
            Read full review
          </button>
        )}
...
const GoogleReviews = () => {
  ...
  const targetSpeedRef = useRef(BASE_SCROLL_SPEED);
  const setWidthRef = useRef(0);

  useEffect(() => {
    const updateWidth = () => {
      setWidthRef.current = singleSetRef.current?.scrollWidth ?? 0;
    };
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !marqueeTrackRef.current) {
      return;
    }
    ...
    const animate = (time: number) => {
      ...
      currentSpeedRef.current += (targetSpeedRef.current - currentSpeedRef.current) * Math.min(1, delta * 0.008);

      if (setWidthRef.current > 0) {
        offsetRef.current = (offsetRef.current + currentSpeedRef.current * delta) % setWidthRef.current;
  ...
  useEffect(() => {
    targetSpeedRef.current = selectedReview ? 0 : BASE_SCROLL_SPEED;
  ...
  const resumeScroll = () => {
    if (!selectedReview) {
      targetSpeedRef.current = BASE_SCROLL_SPEED;
    }
  };

  return (
    <section className="section-padding overflow-hidden bg-secondary/50">
      <div className="container-luxury">
        ...
        >
          <div className="mb-6 flex items-center justify-center gap-2">
            <div className="flex h-6 w-6 items-center justify-center rounded-full border border-gold/40 bg-background text-[0.65rem] font-sans font-semibold text-gold">
              G
            </div>
          ...
          <span className="mb-4 inline-block text-sm font-sans uppercase tracking-[0.2em] text-gold">
            Patient Reviews
          </span>
        ...
        <motion.div
          ...
          className="relative"
        >
          <div className="pointer-events-none absolute inset-y-0 left-0 z-10 hidden w-20 bg-gradient-to-r from-secondary/90 to-transparent lg:block" />
          <div className="pointer-events-none absolute inset-y-0 right-0 z-10 hidden w-20 bg-gradient-to-l from-secondary/90 to-transparent lg:block" />

          <div className="overflow-hidden" onMouseEnter={pauseScroll} onMouseLeave={resumeScroll}>
            <div ref={marqueeTrackRef} className="flex w-max will-change-transform">
              <div ref={singleSetRef} className="flex gap-4 pr-4">
                {reviews.map((review) => (
                  <ReviewCard key={review.name} review={review} onReadMore={setSelectedReview} />
                ))}
              </div>
              <div className="flex gap-4 pr-4">
                {reviews.map((review) => (
                  <ReviewCard key={`${review.name}-duplicate`} review={review} onReadMore={setSelectedReview} />
                ))}
              </div>
            </div>
          </div>
        ...
        {selectedReview && (
          ...
          >
            <motion.div
              ...
              className="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-sm border border-border bg-card p-8 shadow-luxury-lg"
              onClick={(event) => event.stopPropagation()}
            >
              <button
                type="button"
                aria-label={`Close full review from ${selectedReview.name}`}
                onClick={() => setSelectedReview(null)}
              ...
              <div className="mb-6 flex items-center gap-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-full border border-gold/40 bg-secondary text-sm font-sans font-semibold text-gold">
                  G
                </div>
                <div>
                  <p className="font-sans text-sm font-medium text-foreground">{selectedReview.name}</p>
                  <p className="text-xs font-sans uppercase tracking-[0.16em] text-muted-foreground">
                    Google Review · {selectedReview.date}
                  </p>
                </div>
              </div>

              <ReviewStars size={18} />
              <p className="body-large leading-relaxed text-foreground/80 italic">
                "{selectedReview.text}"
              </p>
            </motion.div>
          </motion.div>
